...
export default function MemberAccountDetails() {
    ...
    useEffect(() => {
        ...
        GetMemberTransaction(details, async (res) => {
    ...
    return <section className="h-screen w-full">
        {request ? <FullPageLoader /> : <div>
            <FilteredTable data={data} table_head={render_head(data)} render_body={(a)=>renderTable_body(a)} page={page} prev={previousPage} next={nextPage} />
        </div>}
    </section>
}