...
export function FilteredTable({ data, table_head, render_body, page, prev, next }) {
  ...
  function SearchDetails() {
    if (!data) return;
    return data.filter((data) => (Object.values(data).toString().toLowerCase().includes(value.toLowerCase())))
  }
  ...
  return <div className="overflow-x-auto relative shadow-md sm:rounded-lg">
    ...
    <div className="relative mt-1">
        ...
        <input type="text" onChange={(e)=> setValue(e.target.value)} id="table-search" class="block p-2 pl-10 w-80 text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 " placeholder="Search for items"/>
    </div>
    </div>
    ...
    <nav className="flex justify-end items-center bg-white p-4" aria-label="Table navigation">
       
        <ul className="inline-flex items-center -space-x-px">
          <li>
            <button type="button" disabled={page === 1 ? true: false} onClick={prev} className="flex py-2 px-3 ml-0 leading-tight text-gray-700 bg-white rounded-l-lg border border-gray-300 hover:bg-gray-100 hover:text-gray-700 cursor-pointer">
